...
export default class WorkItem extends React.PureComponent {
  ...
  getLinkTags () {
    ...
    if (this.props.work.appUrl) {
      linkTags[linkTags.length] = (
        <a href={this.props.work.appUrl} key={linkTags.length} target='_blank' rel='noopener' title={`${this.props.work.title} App Link`} className='flex flex-dc flex-full-center'>
          <Icon iconName='app' />
          <p>{langContext.app}</p>
        </a>
      )
